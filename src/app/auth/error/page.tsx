...
const ERROR_MESSAGE: Record<string, { title: string; detail: string }> = {
  OAuthCallback:       { title: 'เกิดข้อผิดพลาดระหว่างเข้าสู่ระบบ', detail: 'กำลังเข้าสู่ระบบใหม่อัตโนมัติ…' },
...
// Errors that can be resolved by simply retrying signin (fresh state cookie)
const AUTO_RETRY_ERRORS = new Set(['OAuthCallback', 'OAuthSignin'])
...
function ErrorContent() {
  ...
  // Auto-retry once for transient OAuth errors (e.g. state mismatch after signout)
  const [retrying, setRetrying] = useState(false)

  useEffect(() => {
    if (!AUTO_RETRY_ERRORS.has(errorCode)) return

    const RETRY_KEY = `oauth_retried_${errorCode}`
    if (sessionStorage.getItem(RETRY_KEY)) {
      // Already retried once — show manual button, don't loop
    ...
    setRetrying(true)
    const t = setTimeout(() => signIn('line', { callbackUrl: '/' }), 1000)
    return () => clearTimeout(t)
  }, [errorCode]) // eslint-disable-line react-hooks/exhaustive-deps

  const isPermanent = !AUTO_RETRY_ERRORS.has(errorCode)
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          background: retrying ? 'var(--ok-dim)' : 'var(--danger-dim)',
          border: `1px solid ${retrying ? 'rgba(22,163,74,.2)' : 'rgba(220,38,38,.2)'}`,
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          margin: '0 auto 20px', transition: 'all .3s',
        }}>
          {retrying
            ? (
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="var(--ok-text)" strokeWidth="2" strokeLinecap="round">
        ...
        <div style={{ fontSize: 20, fontWeight: 700, color: 'var(--text-primary)', marginBottom: 10 }}>
          {retrying ? 'กำลังเข้าสู่ระบบ…' : err.title}
        </div>
        <div style={{ fontSize: 14, color: 'var(--text-muted)', lineHeight: 1.7, marginBottom: 28 }}>
          {retrying ? 'ระบบกำลังพาท่านกลับไปยังหน้าเข้าสู่ระบบอัตโนมัติ' : err.detail}
        </div>

        {/* Error code badge (only when not auto-retrying) */}
        {!retrying && errorCode && (
          <div style={{
        ...
        {/* Manual retry button (shown when not auto-retrying, or for permanent errors) */}
